...
import { deleteStoredOrder, loadOrders, normalizeOrder, saveOrder, subscribeToOrders, updateStoredOrder } from './lib/orders';
...
export default function App(){
 ...
 const confirm=async details=>{
  const preview=await fileData(details.receipt),order={order_number:String(Date.now()).slice(-8),items:cart.map(item=>({product:item.product,quantity:item.quantity,price:item.variant.price,variant:item.variant.label})),total:details.total,fulfillment:details.mode,customer_name:details.form.name,customer_phone:details.form.phone,address:details.form.address,status:'Pedido recibido',stage:0,receipt_name:details.receipt?.name||'',receipt_preview:preview,created_at:new Date().toISOString()};

...
export default function App(){
 ...
 const deleteOrder=async id=>{const previous=orders,previousCustomer=customerOrders;setOrders(items=>items.filter(order=>order.id!==id));setCustomerOrders(items=>items.filter(order=>order.id!==id));if(latestOrder?.id===id)setTrackOpen(false);const{error}=await deleteStoredOrder(id);if(error){setOrders(previous);setCustomerOrders(previousCustomer);window.alert(`No se pudo eliminar el pedido: ${error.message}`);return false}return true};
